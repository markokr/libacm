...
static int bits_flush(BitsEncoder *bits, bool partial)
{
	while (bits->count >= 8) {
...
static int bits_write(BitsEncoder *bits, uint32_t val, unsigned int n_bits)
{
	...
	bits->buf |= (val & mask) << bits->count;
	bits->count += n_bits;

	return bits->count >= 8 ? bits_flush(bits, false) : 0;
}
...
typedef struct {
	ReadSampleFunction *reader_func; /* sample reader */
	void *reader_arg;		 /* argument for reader_func */
	bool reader_eof;		 /* true if eof was hit */

	BitsEncoder bits;    /* output writer */
	long acm_start_ofs;  /* acm header */
	long wavc_start_ofs; /* wavc header */
	bool wavc;	     /* is wavc file */

	uint32_t sample_count; /* nubmer of samples accepted */
	bool enable_output;    /* disable output while priming */
	...
	int quant_power;	/* bits needed to represent max word used */
...
static const Format map_fmt_zz[MAX_Z + 1] = { ZeroFill, Peak1ZZ, Peak2ZZ, Peak3ZZ, Peak4ZZ };
static const Format map_fmt_z[MAX_Z + 1] = { ZeroFill, Peak1Z, Peak2Z, Peak3Z, Peak4Z };
static const Format map_fmt_flat[MAX_FLAT + 1] = {
	ZeroFill, Peak1Base3, Peak2Base5, Binary3, Peak5Base11, Peak5Base11,
};
...
static int calc_cost_z(Encoder *enc, int col, int32_t abs_peak, Format *res_col_fmt)
{
...
static int calc_cost(Encoder *enc, int col, Format *res_col_fmt)
{
	int32_t min_word = INT16_MAX;
	int32_t max_word = INT16_MIN;
	for (int row = 0; row < enc->n_rows; row++) {
		int32_t w = codeword(enc, row, col);
		if (w < min_word) {
			min_word = w;
	...
	int32_t abs_peak = abs(max_word) > abs(min_word) ? abs(max_word) : abs(min_word);

	int cost = calc_cost_flat(enc, abs_peak, min_word, max_word, res_col_fmt);

	if (abs_peak > 0 && abs_peak <= MAX_Z) {
		Format fmt_z;
		int cost_z = calc_cost_z(enc, col, abs_peak, &fmt_z);
		if (cost_z <= cost) {
			cost = cost_z;
			*res_col_fmt = fmt_z;
		}
...
/*
 * Estimate rate and set per-subband format.
 */
static int estimate_bits(Encoder *enc, int step)
{
	Format fmt;
	int bits = 4 + 16 + enc->n_columns * 5; /* block and column headers */
	...
	for (int col = 0; col < enc->n_columns; col++) {
		bits += calc_cost(enc, col, &fmt);
		enc->column_format[col] = fmt;

		if (fmt > Binary3 && fmt <= Binary16) {
			int nbits = fmt;
			if (enc->quant_power < (nbits - 1)) {
				enc->quant_power = nbits - 1;
			}
		}
	}
 ...
 */
static void transform_subband(Encoder *enc, const float *src, float *dst, int cols, int rows)
{
...
static void transform(Encoder *enc)
{
	...
	for (int i = 0; i < enc->n_levels; i++) {
		...
		for (int col = 0; col < cols; col++) {
			transform_subband(enc, src + col, dst + col, cols, rows);
		}
...
static int flush_encoder(Encoder *enc)
{
	...
	return bits_flush(&enc->bits, true);
}
...
static int process_audio(Encoder *enc)
{
	...
	return flush_encoder(enc);
}

static int setup_encoder(Encoder *enc, int levels, int n_rows, float comp_ratio,
			 const Filter *filter)
{
	enc->n_levels = levels;
	...
	enc->bit_budget = SAMPLE_WIDTH * enc->block_len * comp_ratio;
...
int32_t acm_encode(ReadSampleFunction *read_func, void *read_arg, FILE *out, uint16_t channels,
		   ...
		   float comp_ratio, int wavc)
{
	...
	if (comp_ratio > 1 || comp_ratio < 1.0 / 100)
		return ACM_ERR_BADFMT;
	...
	enc->reader_func = read_func;
	enc->reader_arg = read_arg;
	enc->volume = volume;
	enc->wavc = wavc;
	bits_init(&enc->bits, out);

	int err = setup_encoder(enc, levels, samples_per_subband, comp_ratio, &std_filter);
	if (err)
		goto error;
